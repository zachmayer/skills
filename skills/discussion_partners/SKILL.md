...
Where `SKILL_DIR` is the directory containing this skill. The script uses PEP 723 inline metadata so it is self-contained — no project venv needed. The `-m` flag takes a full [pydantic-ai model string](https://ai.pydantic.dev/api/models/) — the provider prefix determines which API key and thinking settings to use.
