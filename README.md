...
- **OAuth token lifecycle** (2026-02): `claude setup-token` can invalidate interactive sessions because OAuth refresh tokens are single-use — when one instance refreshes, the other's refresh token is invalidated. The heartbeat avoids this by using a ~1 year `setup-token` that rarely needs refreshing. Key facts: (1) `ANTHROPIC_API_KEY` has auth priority 1, `CLAUDE_CODE_OAUTH_TOKEN` is priority 2; (2) macOS Keychain entry `Claude Code-credentials` is shared across all profiles; (3) in fresh containers, also need `~/.claude.json` with `hasCompletedOnboarding: true`; (4) never set both `ANTHROPIC_API_KEY` and `CLAUDE_CODE_OAUTH_TOKEN` — causes double-billing. See obsidian note: `Claude Code Token Lifecycle.md`.
