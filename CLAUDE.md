...
- **Don't trust training data for library APIs.** Your knowledge cutoff may reflect a beta, a different version, or simply be wrong. Always verify the installed API: `uv run python -c "from lib import Class; print(dir(Class()))"` before changing code. If you "know" an API changed but can't reproduce the error, your training data is stale — trust the installed version.
