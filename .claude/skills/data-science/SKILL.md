...
name: data-science
description: >
  Opinionated data science defaults: XGBoost, Polars, scikit-learn, nested
  CV. Use when the user says "build a model", "EDA", "feature engineering",
  "train", "predict", "classification", "regression", "clustering", or
  works with CSV/Parquet data for ML. Enforces: no SHAP package (use
  pred_contribs), no SVMs, no TensorFlow, no Plotly, no SMOTE. Covers
  modeling, validation, metrics, and unsupervised learning. Do NOT use for
  general software engineering, non-ML Python, or data visualization only.
---
