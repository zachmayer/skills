...
name: ultra-think
description: >
  Activates extended deep thinking for maximum reasoning depth. Use when
  facing complex architectural decisions, subtle bugs, multi-step reasoning,
  or any task where correctness matters more than speed. Triggers on "think
  hard", "think deeply", "beast mode", "go autonomous", "take your time",
  "don't rush", or when the user gives a large open-ended task to fully
  solve without hand-holding. Do NOT use for simple questions, quick
  lookups, or trivial edits.
---

ultrathink

Also apply the `mental-models` skill to select and use relevant thinking frameworks.

Also apply the `ask-questions` skill to identify and resolve ambiguity before starting work.
