...
name: private-repo
description: >
  Creates or connects a private GitHub repository for backing up sensitive local
  data (memory, notes, config). Use when another skill needs git-backed storage
  with a private remote, when the user says "back up my notes", "sync my data",
  "set up a private repo", "push my vault", or when you detect a ~/claude
  directory without a remote. Do NOT use for public repos or code projects.
