...
setup-heartbeat-token: ## Generate OAuth token for heartbeat (interactive, one-time)
	...
	@echo ""
	@echo "WARNING: setup-token may invalidate your current interactive session."
	@echo "OAuth refresh tokens are single-use — generating a new token can revoke"
	@echo "the old one. You may need to run /login again in your interactive session."
	@echo ""
