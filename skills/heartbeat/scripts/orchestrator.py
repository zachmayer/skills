...
def invoke_agent(workdir, prompt, issue_number, repo):
    """Invoke Claude Code agent in a new process group. Returns exit code.

    The process group ensures the watchdog (which kills the orchestrator)
    also kills the claude subprocess — preventing orphan processes.
    """
    ...
    with open(log_file, "w") as lf:
        ...
        try:
            ...
            # If orchestrator is killed, take down the entire process group
