...
description: >
  Evolves prompts through genetic-style optimization using parallel sub-agents,
  LLM-as-judge scoring, and iterative selection. Generates variants via mutation
  and crossover, scores them against test inputs, and returns the highest-fitness
  prompt. Use when the user says "optimize this prompt", "improve my prompt",
  "make this prompt better", "evolve this prompt", or needs systematic prompt
  tuning with a clear evaluation criterion. Do NOT use for one-off prompts,
  simple rewording, or when the criterion is unclear.
allowed-tools: Bash(uv run *)
---

Optimize prompts through evolutionary search: generate variants, score with `llm-judge`, keep the fittest, mutate and recombine, repeat.
...
- **Judge criterion**: a specific, binary pass/fail criterion (see `llm-judge` skill)
- **Generations**: how many rounds to run (default: 3)
- **Population size**: variants per generation (default: 4)

### 2. Generate variants

Launch parallel sub-agents, each using `discussion-partners` to produce one variant. Apply two genetic operators:
...
1. Run the variant prompt against a test model (via `discussion-partners`)
2. Score the output using `llm-judge` with the defined criterion
...
1. **Variant generation**: parallel sub-agents, each uses `discussion-partners` to create one variant
2. **Output generation**: parallel sub-agents, each runs one variant × one test input via `discussion-partners`
3. **Judging**: parallel sub-agents, each scores one output via `llm-judge` pattern
