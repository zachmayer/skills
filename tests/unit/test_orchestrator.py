...
def test_build_prompt_has_constraints():
    """Worker prompt includes constraints section."""
    prompt = orchestrator.build_prompt(
        issue_number=1,
        repo="x/y",
        issue_title="t",
...
def test_build_prompt_lint_retry_cap():
    """Worker prompt includes retry guidance for lint/test."""
    prompt = orchestrator.build_prompt(
        issue_number=1,
        repo="x/y",
        issue_title="t",
    ...
    # Should mention lint and test
    assert "make lint" in prompt.lower()
    assert "make test" in prompt.lower()
