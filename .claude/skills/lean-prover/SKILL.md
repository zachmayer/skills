...
description: >
  ...
  Use when the user says "prove", "formalize", "Lean", "Mathlib", "theorem",
  "lemma", or mentions competition problems (Putnam, IMO, MiniF2F). Also use
  for orchestrating complex proof campaigns across multiple lemmas.
  Do NOT use for general programming, non-Lean math, or single-tactic proofs
  that need no orchestration.
...
Consult an external model via the `discussion-partners` skill at these attempt counts:
