...
- **Notes with personal facts** — if a note records something that could have changed (current project, team structure, tool preferences), ask the user to confirm rather than assuming. See the [hierarchical-memory.md](hierarchical-memory.md) Fact Freshness section for velocity guidance.
...
| Daily note append (respects daily note plugin config) | Memory files (managed by knowledge-system/hierarchical-memory) |
...
- Memory files in `memory/` are managed by `knowledge-system` — don't edit directly
